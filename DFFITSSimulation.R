...
BestDFFITS_fun<-function(Xtmat,Ytmat,BasisNum = 15){
  ...
  for(i in n){
    k=1
    for(j in n[-i]){ #go through all observations except the ith
      yHatsNoi[,k]<-B0_i_[,i]+B1_i_[,i]*Xtmat[,j] #Gets yHat for all N-1 observations with i removed
      k=k+1
    }
    MSE_mini_t[,i]<-apply((Ytmat[,-i]-yHatsNoi)^2,1,mean) #Get an MSE across the other events when event i is left out
  }
  altDFFITS<-matrix(NA,nrow=nrow(Xtmat),ncol=ncol(Xtmat))
  for(t in 1:nrow(Xtmat)){ #at each timepoint calculate equation from pg 396/401 of kutner
    altDFFITS[t,]<- (FullResults$Residuals[t,]/sqrt(MSE_mini_t[t,]*(1-FullResults$AdjHatMatrix[,t])))*sqrt(FullResults$AdjHatMatrix[,t]/(1-FullResults$AdjHatMatrix[,t]))
  }
  out<-list(altDFFITS)
...
absdffit_bootsamedata<-function(alpha, xdata = sim_data$Xdata, outlier = sim_data$OutlierNumber,ydata = sim_data$Ydata, nboot = 100){
  ...
  raw_dffits<-BestDFFITS_fun(Xtmat = xdata,Ytmat = ydata,BasisNum = 15) #pointwise DFFITS for each observation
  mean_abs_dffits<-apply(abs(raw_dffits$altdffits),2,mean)
  adjusted_obs_absdffits<-mean_abs_dffits[outlier]
  theta<-as.vector((1/mean_abs_dffits)^alpha/sum((1/mean_abs_dffits)^alpha))
  ...
  for(i in 1:nboot){
    ...
    temp_dffits<-BestDFFITS_fun(Xtmat = xdata[,index_samp],Ytmat = ydata[,index_samp],BasisNum = 15)$altdffits
    boot_meanabsdffits<-c(boot_meanabsdffits,apply(abs(temp_dffits),2,mean))
  }
  percent_95<-quantile(boot_meanabsdffits,0.95)
  percentile <- ecdf(boot_meanabsdffits)
  adjusted_point_percentile<- percentile(mean_abs_dffits[outlier])
  
  out<-list(percent_95,adjusted_point_percentile,mean_abs_dffits) #returns the 95th percentile from the N*nboot |dffits| calculated
  names(out)<-c("Boot95","AdjPercentile","AllObsMeanAbsDFFITS") #And returns the percentile of this list that the observed |dffits| falls into
  return(out)
}
...
for(l in lambda){
  ...
  for(a in alpha){
   tempout<-absdffit_bootsamedata(xdata = sim_data$Xdata, ydata = sim_data$Ydata, outlier = sim_data$OutlierNumber, alpha = a,  nboot = nboot)
    results_output95[i,j]<- tempout$Boot95
      results_outputpercentile[i,j]<- tempout$AdjPercentile
      results_numberobs_over95[i,j] <-sum(ifelse(tempout$AllObsMeanAbsDFFITS>=tempout$Boot95,1,0))
