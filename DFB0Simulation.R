...
absDFB0_bootsamedata<-function(alpha, xdata = sim_data$Xdata,ydata = sim_data$Ydata, outlier = sim_data$OutlierNumber, nboot = 100){
  ...
  raw_DEFBETAS0<-DFBETAS_fun(Xtmat = xdata, Ytmat = ydata,BasisNum = 15)$DFBETAS0t #pointwise DFBETAS0 for each observation
  mean_abs_dfbetas0<-apply(abs(raw_DEFBETAS0),2,mean)
  adjusted_obs_absdfbetas0<-mean_abs_dfbetas0[outlier]
  ...
  boot_meanabsdfbetas0<-c()
  for(i in 1:nboot){
    index_samp<-sample(1:N,N,replace=TRUE,prob = theta) #resamples the needed data with the given probability
    temp_dfbetas0<-DFBETAS_fun(Xtmat = xdata[,index_samp],Ytmat = ydata[,index_samp],BasisNum = 15)$DFBETAS0t
    boot_meanabsdfbetas0<-c(boot_meanabsdfbetas0,apply(abs(temp_dfbetas0),2,mean))
  }
  percent_95<-quantile(boot_meanabsdfbetas0,0.95) #get 95th percentile from this run
  percentile <- ecdf(boot_meanabsdfbetas0)
  adjusted_point_percentile<- percentile(mean_abs_dfbetas0[outlier]) #gets percentile of the originally adjusted observation

  out<-list(percent_95,adjusted_point_percentile,mean_abs_dfbetas0) #returns the 95th percentile from the N*nboot |dffits| calculated
  names(out)<-c("Boot95","AdjPercentile","AllObsMeanAbsDFBETAS0") #And returns the percentile of this list that the observed influence measure falls into
  return(out)
}

one_fulltable_bootstrap<-function(N = 30, nboot = 50, alpha, lambda, length = 1000){
  ...
  for(l in lambda){
    ...
    for(a in alpha){
      tempout<-absDFB0_bootsamedata(xdata = sim_data$Xdata, ydata = sim_data$Ydata, outlier = sim_data$OutlierNumber, alpha = a,  nboot = nboot)
      results_output95[i,j]<- tempout$Boot95
      results_outputpercentile[i,j]<- tempout$AdjPercentile
      results_numberobs_over95[i,j] <-sum(ifelse(tempout$AllObsMeanAbsDFBETAS0>=tempout$Boot95,1,0))
...
filename95=paste("~/InfluenceMeasures/data/DFB0_avg_95_",iter,".RData",sep="")
x<-hold$the95percentile
save(x, file=filename95)

filenamepercentile=paste("~/InfluenceMeasures/data/DFB0_avg_adjpercentile_",iter,".RData",sep="")
y<-hold$outlierpercentile
save(y, file=filenamepercentile)

filenameOver95=paste("~/InfluenceMeasures/data/DFB0_avg_numberobs_over95_",iter,".RData",sep="")
z<-hold$numberobs_over95
save(z, file=filenameOver95)

filenameAdjpercentileover95=paste("~/InfluenceMeasures/data/DFB0_Above_95_",iter,".RData",sep="")
