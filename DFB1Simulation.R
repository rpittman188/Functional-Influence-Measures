#This is the Code for |DFBETAS1| Simulation and does 1 iteration
...
BetaOnlyBSpline<-function(xData,yData,BasisNum=15, BetaOnly=FALSE,lambda=10^(-1)){
  n<-nrow(xData)
  nPred<-ncol(xData) #Number of predictors 
  ...
  if(BetaOnly==FALSE){
    ...
    cedhatmat = eval.fd(gaittime, cedhatfd)  #1 to n compare this to yHat.t.mat
    resmat. = mygaitExp[,,2] - cedhatmat #The 2 represents response curve
    SigmaE = cov(t(resmat.))
...
DFBETAS_fun<-function(Xtmat,Ytmat,BasisNum = 15){
  ...
  DFBETAS0<-matrix(NA,nrow=nrow(Xtmat),ncol=ncol(Xtmat))
  DFBETAS1<-matrix(NA,nrow=nrow(Xtmat),ncol=ncol(Xtmat))
  ...
  for(i in 1:ncol(Xtmat)){ #for each event
    DFBETAS0[,i]<-(FullResults$B0t-B0_i_[,i])/stderrB0_i_2[,i]
    DFBETAS1[,i]<-(FullResults$B1t-B1_i_[,i])/stderrB1_i_2[,i]
    # DFBETA011[,i]<-(FullResults$B0t-B0_i_[,i])
    # DFBETA111[,i]<-(FullResults$B1t-B1_i_[,i])
  }
  

  out<-list(DFBETAS0,DFBETAS1)
  names(out)<-c("DFBETAS0t","DFBETAS1t")
  return(out)
} #Returns DFBETAS0t and DFBETAS1t for each of the N events. Each event is a column

#generate data before using this function. This function bootstraps given data
##FOR DFBETAS0 change to 1
absDFB1_bootsamedata<-function(alpha, xdata = sim_data$Xdata,ydata = sim_data$Ydata, outlier = sim_data$OutlierNumber, nboot = 100){
  ...
  raw_DEFBETAS1<-DFBETAS_fun(Xtmat = xdata, Ytmat = ydata,BasisNum = 15)$DFBETAS1t #pointwise DFBETAS0 for each observation
  mean_abs_dfbetas1<-apply(abs(raw_DEFBETAS1),2,mean)
  adjusted_obs_absdfbetas1<-mean_abs_dfbetas1[outlier]
  theta<-as.vector((1/mean_abs_dfbetas1)^alpha/sum((1/mean_abs_dfbetas1)^alpha))
  #Need to set up the bootstrap next SEE OrdinaryRegressionDistribution for Details
  #Set blank objects here
  boot_meanabsdfbetas1<-c()
  for(i in 1:nboot){
    index_samp<-sample(1:N,N,replace=TRUE,prob = theta) #resamples the needed data with the given probability 
    temp_dfbetas1<-DFBETAS_fun(Xtmat = xdata[,index_samp],Ytmat = ydata[,index_samp],BasisNum = 15)$DFBETAS1t
    boot_meanabsdfbetas1<-c(boot_meanabsdfbetas1,apply(abs(temp_dfbetas1),2,mean))
  }
  percent_95<-quantile(boot_meanabsdfbetas1,0.95) #get 95th percentile from this run
  percentile <- ecdf(boot_meanabsdfbetas1)
  adjusted_point_percentile<- percentile(mean_abs_dfbetas1[outlier]) #gets percentile of the originally adjusted observation
  
  out<-list(percent_95,adjusted_point_percentile,mean_abs_dfbetas1) #returns the 95th percentile from the N*nboot |dffits| calculated
  names(out)<-c("Boot95","AdjPercentile","AllObsMeanAbsDFBETAS1") #And returns the percentile of this list that the observed |dffits| falls into
  return(out)
}
...
one_fulltable_bootstrap<-function(N = 100, nboot = 100, alpha, lambda, length = 1000){
  ...
  for(l in lambda){
    ...
    for(a in alpha){
      tempout<-absDFB1_bootsamedata(xdata = sim_data$Xdata, ydata = sim_data$Ydata, outlier = sim_data$OutlierNumber, alpha = a,  nboot = nboot)
      results_output95[i,j]<- tempout$Boot95
      results_outputpercentile[i,j]<- tempout$AdjPercentile
      results_numberobs_over95[i,j] <-sum(ifelse(tempout$AllObsMeanAbsDFBETAS1>=tempout$Boot95,1,0))
  ...
  ##NEW I also want this function to return a table saying 0/1 if the adjusted observation
  #is above the 95%ile should be able to get that from the results_outputpercentile table
  #I also want to be able to return a histogram of 1-percentile of adjusted observation
  #I want the number of times any other observation is above the 95th percentile as well
...
filename95=paste("~/InfluenceMeasures/data/DFB1_avg_95_",iter,".RData",sep="")
x<-hold$the95percentile
save(x, file=filename95)

filenamepercentile=paste("~/InfluenceMeasures/data/DFB1_avg_adjpercentile_",iter,".RData",sep="")
y<-hold$outlierpercentile
save(y, file=filenamepercentile)

filenameOver95=paste("~/InfluenceMeasures/data/DFB1_avg_numberobs_over95_",iter,".RData",sep="")
z<-hold$numberobs_over95
save(z, file=filenameOver95)

filenameAdjpercentileover95=paste("~/InfluenceMeasures/data/DFB1_Above_95_",iter,".RData",sep="")
